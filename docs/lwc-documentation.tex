...
\startitemize
    ...
    \item \Lwc/ will rarely raise a \q{\tt Circular node list detected!} warning. This occurs when the linked list for the replacement paragraph loops back on itself, meaning that there is neither a beginning nor end to the paragraph. Because of this, there is no way to splice the paragraph into the page, therefore \lwc/'s only option is to stop processing the page, without removing the \woo/.
\stopitemize
