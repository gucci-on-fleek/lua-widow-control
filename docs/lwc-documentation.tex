...
\Lwc/ is a Plain~\TeX/\LaTeX/\ConTeXt\ package that removes most \waos/ from a document, \emph{without} stretching any glue or shortening any pages. It does so by automatically lengthening a paragraph on a page where a widow or orphan would otherwise occur. 

\warning{\Lwc/ uses Lua callbacks to modify the internal structure of each page. This typically causes no issues; however, in \bold{rare} scenarios, it may delete or duplicate random paragraphs or lines. All known bugs that cause this issue have been corrected; nevertheless, I recommend against using this package for exams or any document where minor textual changes would be disastrous. This package should be sufficiently stable for use in typical documents.}
...
It is tricky to understand how \lwc/ works if you aren't familiar with how \TeX{} breaks pages. Chapter~15 of \titlecite[texbook] (\quotation{How \TeX{} Makes
...
There have been a few previous attempts to improve upon \TeX's builtin widow and orphan-handling abilities; however, none of these have been able to automatically remove \waos/ without stretching any glue or shortening any pages.
...
Although \TeX{}'s page breaking algorithm is quite simple, it can lead to some fairly complex behaviors in regards to \waos/. The usual choices are to either ignore them, stretch some glue, or shorten the page. \in{Table}[tab:demo] on the next page shows a visual demonstration of some of these behaviors and how \lwc/ differs from the defaults.
...
\startitemize
    ...
    \item Sometimes a widow or orphan just cannot be eliminated because no paragraph has enough \quotation{stretch}. This can usually be remediated by increasing \lwc/'s \estretch/.

    \item When running under \LuaMetaTeX, the log may be filled with lines like \quotation{\tt luatex warning  > tex: left parfill skip is gone}. This is harmless and can be ignored (although I have no idea how to prevent it).
\stopitemize
