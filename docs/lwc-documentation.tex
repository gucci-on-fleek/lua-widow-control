...
\startitemize
    ...
    \item In multicolumn layouts, \lwc/ will remove widows and orphans that occur between pages, but it will not remove widows and orphans between pages. 
    ...
    \item Sometimes a widow or orphan just cannot be eliminated because no paragraph has enough \quotation{stretch}. This can usually be remediated by increasing \estretch/.

    \item When running under \LuaMetaTeX, the log may be filled with lines like \quotation{\tt luatex warning  > tex: left parfill skip is gone}. These are harmless and can be ignored (although I have no idea how to prevent them).
\stopitemize
