...
Lines into Pages}) is the best reference for this; however, it goes into much more detail than most users require. If you are unable to access \cite[title][texbook], then I can also recommend \sectionmark27 of \titlecite[texbytopic], \goto{available online}[url(https://texdoc.org/serve/texbytopic/0#page=227)] for free. Below follows a simplified (and likely error-ridden) summary of \TeX{}'s page breaking algorithm:
...
Eventually, \lwc/ will be on \acronym{CTAN} and in \TeX{}Live and Mik\TeX{}; until then, you will need to manually install the package. The procedure should be fairly similar regardless of your \acronym{OS}, \TeX{}  distribution, or format.
...
\startitemize
    ...
    \item In multicolumn layouts, \lwc/ will remove \waos/ that occur between pages, but it will not remove \waos/ between columns.  To do so would likely require modifying the output routine, a complicated endeavour well beyond my \TeX{}nical expertise.
    ...
    \item \Lwc/ will not expand the first paragraph of a document. This should never realistically be an issue.
\stopitemize
...
At this point, we know that we have a \woo/ on the page. Now, we iterate through the list of saved paragraphs to find the lengthened paragraph with the least demerits. Once we've selected a paragraph to replace, we can traverse through the page to find the original version of this paragraph so that we can splice in the lengthened paragraph. 
